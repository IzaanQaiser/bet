...
Writing a placeholder is still a real Calendar write, so it must preserve ADR 0003's boundary: exactly one service ever calls the Calendar write API. `committer-svc` stays the sole writer. `dispatcher-svc` (which owns all Calendar *reads* and all `next_fit_start` computation) requests the write via a synchronous call, `PUT /latents/{item_id}/placeholder` / `DELETE /latents/{item_id}/placeholder` on committer-svc — the second synchronous cross-service asymmetry in this codebase (the first being `ingest-svc`'s direct forward to resolver-svc/dispatcher-svc, `overview.md` §2). A synchronous call, not Pub/Sub, because dispatcher-svc needs the real Calendar event id back immediately, to persist into `latents.placeholder_event_id`.
...
### 5.4 Firing, and the accept/decline/snooze outcomes
...
4. Otherwise: send the text (`converse_suggestion`'s nudge turn, agent-contracts.md §4.2, LLM-generated), open a `suggestions` row (`snapshot_id` left `NULL` — no batch-scoring snapshot exists for this path anymore, `scheduled_for` carries the instant instead — `migrations/0020`).
