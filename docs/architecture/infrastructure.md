...
| `sa-resolver` | subscribe: `items.extracted` · publish: `items.confirmed`, `items.raw` (conversation-continuity fix, below) | yes | none | `roles/aiplatform.user` | none | itself (same push pattern as `sa-extractor`), `sa-ingest` (`roles/run.invoker`, for routed replies) |
| `sa-committer` | subscribe: `items.confirmed`, `items.raw.dlq`, `items.extracted.dlq`, `items.confirmed.dlq` · publish: none | yes | `google-oauth-client-secret`, `user-refresh-token-*` (accessor) | none | Calendar (write), Gmail (send) | itself (same push pattern as `sa-extractor`), `sa-dispatcher` (`roles/run.invoker`, ADR 0009's placeholder `PUT`/`DELETE` call — §5.1) |
| `sa-dispatcher` | subscribe: none (cron-triggered) · publish: `items.confirmed` (accept-path only) | yes | `google-oauth-client-secret`, `user-refresh-token-*` (accessor) | none | Calendar (read only) | Cloud Scheduler (`roles/run.invoker`), `sa-ingest` (routed suggestion replies), `sa-committer`/`sa-dashboard` (both `roles/run.invoker` on itself, for their own next-fit-recompute Cloud Tasks — §5.1), developer (manual trigger, §5) |
...
**Resolved gap, found live-testing the conversation-continuity fix (Phase G follow-up, same session as step D) — `sa-resolver` needed two grants it never had before, neither anticipated up front.** Before this fix, `sa-resolver` only ever mutated `items` rows `sa-ingest` had already created (`SELECT, UPDATE`, migration 0001) and only ever published to `items.confirmed`. The fix has `resolver-svc` originate a brand-new item itself (when a reply doesn't relate to the one it was routed against) — the same shape a first-contact message takes — which needs both `INSERT` on `items` (`migrations/0008_grant_resolver_items_insert.sql`) and `roles/pubsub.publisher` on `items-raw` (`infra/pubsub.tf`'s `resolver_publishes_raw` resource). Both gaps surfaced as real 500s on the first two live test attempts (`permission denied for table items`, then `IAM_PERMISSION_DENIED` on the retry) — not caught by any mocked test, since every existing test mocks the DB connection and `publish()` directly.
...
Every Pub/Sub grant above is bound **at the topic or subscription resource**, not project-wide — a project-wide `roles/pubsub.publisher` on `sa-extractor` would let it publish to `items.confirmed` directly and quietly defeat the typed write-boundary story (ADR 0003). This is non-negotiable in the Terraform: `google_pubsub_topic_iam_member` / `google_pubsub_subscription_iam_member`, resource-scoped, never `google_project_iam_member` for Pub/Sub roles.
...
**Manual OAuth bootstrap — populated during step 6, per PRD §14's scope note.** The OAuth client itself has to be created by hand in GCP Console (OAuth consent screen, External + Testing with the developer's own account as a test user, scopes `calendar.events` + `gmail.send`; Desktop app credential type — no hosted redirect URI needed). The client ID is not a secret (same treatment as the Twilio Account SID, §4.1) and is passed as plain config (`GOOGLE_OAUTH_CLIENT_ID` in `scripts/deploy.sh`'s `committer-svc` case); only the client secret goes into `google-oauth-client-secret`. Once the client exists, `scripts/bootstrap_oauth_token.py <user_id>` runs the consent flow locally (opens a browser, catches the redirect on a local server), mints a refresh token, creates the `user-refresh-token-{user_id}` secret, and updates that user's `google_refresh_token_ref` — the one-time manual step PRD §14 describes, now a runnable script instead of an ad hoc procedure. **Caveat, not yet hit but worth knowing:** an OAuth app in Testing status is limited to its declared test users and — historically, though Google's exact policy here has shifted over time — testing-mode grants have in the past been subject to a refresh-token lifetime shorter than a verified app's. If the demo token stops working, the fix is re-running the bootstrap script, not a code change.
...
### 5.1 Cloud Tasks — a second scheduling primitive alongside Cloud Scheduler, one shared queue

**Gap closed here, not previously documented:** a single Cloud Tasks queue, `reminders` (`us-central1`), provisioned once by hand (not Terraform, not `scripts/deploy.sh` — same "provisioned once by hand" treatment as the OAuth client, §4), is reused for every precise, per-item scheduled trigger in this system — Cloud Scheduler (§5 above) only ever fires on a fixed cron, which is wrong for anything that needs to fire at an *item-specific* instant (a reminder, a next-fit recompute, a fire-time text).

Every Cloud Tasks caller mints its OIDC token *as the target service's own service account* — the identical pattern §2's "Resolved gap" note already established for Pub/Sub push subscriptions, just with Cloud Tasks as the intermediary instead of Pub/Sub. Concretely, the queue currently has four distinct (enqueuer → target) pairs, each needing its own `roles/cloudtasks.enqueuer` (on the queue) + `roles/iam.serviceAccountUser` (on the target SA, letting the enqueuer mint a token *as* it) + `roles/run.invoker` (target SA on itself, satisfying its own OIDC check):

| Enqueues | Target endpoint | Fires |
|---|---|---|
| `sa-committer` | `dispatcher-svc` `/dispatch/reminders/fire` | at the exact obligation reminder instant |
| `sa-committer` | `dispatcher-svc` `/latents/{id}/next-fit` | immediately, on a fresh idea's commit |
| `sa-dashboard` | `dispatcher-svc` `/users/{id}/next-fit` | immediately, on a working-hours change |
| `sa-dispatcher` | `dispatcher-svc` `/latents/{id}/fire` | at the exact `next_fit_start` instant (ADR 0009) — dispatcher-svc's first time enqueueing *for itself*, not just being a target |

**Resolved gap, found live the first time this path actually fired:** the fourth row needs `sa-dispatcher` granted `roles/cloudtasks.enqueuer` on `reminders` and `roles/run.invoker` on itself (already held, since Cloud Scheduler's `/dispatch` trigger needs the same grant) — but that's not sufficient on its own for a *self-referential* mint. Minting an OIDC token naming yourself as both the task-creating principal and the target audience additionally needs `roles/iam.serviceAccountUser` granted by `sa-dispatcher` **to itself** (`gcloud iam service-accounts add-iam-policy-binding sa-dispatcher@... --member=serviceAccount:sa-dispatcher@... --role=roles/iam.serviceAccountUser`) — without it, `enqueue_fire_task` 500'd with `PermissionDenied: ... lacks IAM permission "iam.serviceAccounts.actAs"`. Every other Cloud Tasks caller in this table names a *different* SA as the target, so this requirement never surfaced until dispatcher-svc became its own target (ADR 0009).
...
## 10. Web division addendum (through Phase 5)

A second, later initiative — the public landing/waitlist/registration/dashboard site (own plan doc, not one of the six above) — adds new Cloud Run services on top of everything §1–9 describes, without changing any of it. Tracked here as an addendum rather than folded into the tables above, since the two systems have different owners' worth of scope. §1–9 stay closed.

- **`sa-registration`** (`infra/service_accounts.tf`) — `registration-svc`'s identity. `roles/cloudsql.client` + `roles/cloudsql.instanceUser` (`infra/cloud_sql.tf`), same pattern as the five pipeline services.
- **Postgres roles** — `SELECT, INSERT` on `waitlist` (`migrations/0009_waitlist.sql`) and on `users` (`migrations/0010_registration_grants.sql`). No `UPDATE` on either: approval (Phase 3) runs as the developer's own IAM identity via a local script, the same `google_sql_user.developer_admin` bootstrap §2.2 already describes for migrations, and a phone number only ever becomes a `users` row once (the `phone_e164` UNIQUE constraint is the real guard against double-registration) — `registration-svc` never needs to update either table, only insert.
- **Secret Manager** (`infra/secret_manager.tf`) — `web-session-signing-key` (signs/verifies every token across the registration flow — Phase 3's registration token, Phase 4's oauth-session and oauth-callback-state tokens, and Phase 5's dashboard-svc session tokens, all sharing one key and one purpose-gated verification, `obligation_engine_shared.tokens`), `google-oauth-client-secret-web` (the Web Application OAuth client, distinct from `bootstrap_oauth_token.py`'s Installed App client — a server-side redirect flow needs the other client type), and `twilio-verify-service-sid` (the plan treats this as a secret, unlike the other Twilio SID/Account-SID identifiers elsewhere in this project, which are plain config per §4.1 — kept consistent with the plan rather than special-cased). `sa-registration` reads all three, plus `twilio-api-key-secret` (same credential the plain-SMS senders use, also valid for Verify API calls).
- **`registration_creates_user_secrets`** — the one thing `registration-svc` creates dynamically, mirroring `committer_creates_user_secrets` above but *not* copying its unconditional project-wide `secretmanager.admin` grant: this one carries an IAM condition scoped to the `user-refresh-token-` name prefix, confirmed against Secret Manager's real resource-name format (`projects/<number>/secrets/<id>` — project *number*, not the project-id string Terraform's own resource `id` attribute happens to display, which would have made a naive condition silently never match).
- **`registration-svc` deploy** (`scripts/deploy.sh`) — `--allow-unauthenticated` like `ingest-svc` (the only other public service), but unlike Twilio's signed webhook, there's no request signature to validate here; CORS (`ALLOWED_ORIGINS` env var, restricted to the GitHub Pages origin) and the in-process rate limit are what stand in for that. `min-instances=0`, not `1` — a cold start here delays one waitlist submission or registration step, not a missed inbound SMS. `OAUTH_REDIRECT_URI` uses Cloud Run's own deterministic default URL format (`https://<service>-<project-number>.<region>.run.app`, no random per-revision hash), so it's known and registrable in the Google Console before the service's first deploy.
- **`/register/[token]` is `/register?token=...`** — the plan's original path-segment form can't work: the frontend is a static export (`web/next.config.ts`), which has no server to pre-render an arbitrary runtime token as a dynamic route. `approve_waitlist.py`'s link format and the frontend's `register-flow.tsx` both use the query-param form instead; functionally identical, just compatible with a server-less static host.
- **`sa-dashboard`** — `dashboard-svc`'s identity (Phase 5), same Cloud SQL binding pattern as every other service. `SELECT` across `items`/`conversations`/`messages`/`obligations`/`suggestions` (`migrations/0011_dashboard_grants.sql`), plus `SELECT, UPDATE` on `users` — `UPDATE`, not just `SELECT` as the plan originally wrote it, since `PATCH /me/profile` writes the caller's own timezone/working-hours row. Every `/me/*` query is scoped to the caller in application code (`WHERE user_id = <from the session JWT>`), matching this project's established GCP-IAM-is-the-hard-boundary/in-table-scoping-is-a-software-invariant split (§2) — Postgres GRANTs alone can't express "only your own rows."
- **Login reuses registration's whole OTP mechanism** — same Twilio Verify Service (`twilio-verify-service-sid`), same signing key (`web-session-signing-key`), same `obligation_engine_shared.tokens` purpose-gated verification, just a different `purpose` claim (`dashboard-session` vs. `registration`/`oauth-session`/`oauth-callback`) so a registration-flow token can never be replayed as a dashboard login, or vice versa. `dashboard-svc` only ever *verifies* this key, same read-only treatment as `sa-registration`'s post-Phase-4 access to it.
- **web basePath applies here too** — `web/next.config.ts`'s `basePath: "/bet"` (found live during Phase 4's testing) automatically covers `/dashboard` as well; no separate fix needed.
